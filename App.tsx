...
import { TaiwanIcon, DownloadIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const data = await fetchAnalysisData(schoolName);
  ...
  const handleExampleClick = (school: string) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-brand-text font-sans">
      <main className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="max-w-3xl mx-auto bg-zinc-900/50 p-4 sm:p-6 rounded-xl shadow-lg border border-zinc-800 backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row gap-4">
            <input
              ...
              placeholder="請輸入學校全名，例如：「台北市立建國高級中學」"
          ...
          <div className="text-center mt-4">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-2">
                {exampleSchools.map(school => (
                    <button 
                        key={school} 
                        onClick={() => handleExampleClick(school)}
                        disabled={isLoading || isGeneratingPdf}
